...
class main_win:
    def __init__(self, master):
        ...
        self.txt=tk.Text(self.frame, width=125)
    ...
    def search(self, *event):
        ...
        self.btn_title_abc=tk.Button(self.frame, text='Sort Alphabetically', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, result_dis_sort.to_markdown(index=False, mode='r'))])
        self.btn_title_abc.grid(row=6, column=0, sticky='news')
        self.btn_title_ind=tk.Button(self.frame, text='Sort by Index', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, result_dis.to_markdown(index=False, mode='r'))])
    ...
    def fill_table_all_rooms(self, *args):         
        ...
        if len(idxs_rooms)==1:
            ...
            if idx_room==0:
                ...
                self.btn_all_h_skill=tk.Button(self.frame, text='Skill Books', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, all_h_dis_skill.to_markdown(index=False, mode='r'))])
                ...
                self.btn_all_h_spell=tk.Button(self.frame, text='Spell Tomes', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, all_h_dis_spell.to_markdown(index=False, mode='r'))])
                ...
                self.btn_all_h_reg=tk.Button(self.frame, text='Spell Tomes', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, all_h_dis_reg.to_markdown(index=False, mode='r'))])
                self.btn_all_h_reg.grid(row=5, column=0, columnspan=2, sticky='news')
                #all_h_dis_sort=all_h_dis.sort_values('Title')
                self.btn_title_abc=tk.Button(self.frame, text='Sort Alphabetically', command=lambda:[self.txt_activate(), self.sort_abc()])
                self.btn_title_abc.grid(row=6, column=0, sticky='news')
                self.btn_title_ind=tk.Button(self.frame, text='Sort by Index', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, all_h_dis.to_markdown(index=False, mode='r'))])
                self.btn_title_ind.grid(row=6, column=1, sticky='news')
                #self.txt.config(state=tk.DISABLED)

    def sort_abc(self, *args):
        self.table = self.txt.get(1.0, 'end-1c')
        self.table_sorted=sorted(self.table, key=lambda x: x['Title'])
        self.tbl_srtd=tbl.tabulate(self.table_sorted, headers='keys')
##        self.txt.delete(1.0, tk.END)
##        self.txt.insert(tk.END, self.tbl_srtd)
        print(tbl.tabulate(self.table_sorted, headers='keys'))
    ...
    def fill_table(self, *args):
        ...
        if len(idxs_rooms)==1:
            ...
            if idx_room==0:
                ...
                house_skill=pd.read_sql_query('SELECT * FROM "My Books" WHERE House=? AND "Skill Book" = "Yes"', con, params=(HOUSE,))
                house_dis_skill=house_skill[['Title', 'House', 'Room']]
                self.btn_house_skill=tk.Button(self.frame, text='Skill Books', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, house_dis_skill.to_markdown(index=False, mode='r'))])
                self.btn_house_skill.grid(row=3, column=0, columnspan=2, sticky='news')
                house_spell=pd.read_sql_query('SELECT * FROM "My Books" WHERE House=? AND "Spell Tome" = "Yes"', con, params=(HOUSE,))
                house_dis_spell=house_spell[['Title', 'House', 'Room']]
                self.btn_house_spell=tk.Button(self.frame, text='Spell Tomes', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, house_dis_spell.to_markdown(index=False, mode='r'))])
                self.btn_house_spell.grid(row=4, column=0, columnspan=2, sticky='news')
                house_reg=pd.read_sql_query('SELECT * FROM "My Books" WHERE House=? AND "Skill Book" = "No" AND "Spell Tome" = "No"', con, params=(HOUSE,))
                house_dis_reg=house_reg[['Title', 'House', 'Room']]
                self.btn_house_reg=tk.Button(self.frame, text='Regular Books', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, house_dis_reg.to_markdown(index=False, mode='r'))])
                self.btn_house_reg.grid(row=5, column=0, columnspan=2, sticky='news')
                query_dis_sort=query_dis.sort_values('Title')
                self.btn_title_abc=tk.Button(self.frame, text='Sort Alphabetically', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, query_dis_sort.to_markdown(index=False, mode='r'))])
                self.btn_title_abc.grid(row=6, column=0, sticky='news')
                self.btn_title_ind=tk.Button(self.frame, text='Sort by Index', command=lambda:[self.txt_activate(), self.txt.insert(tk.END, query_dis.to_markdown(index=False, mode='r'))])
                self.btn_title_ind.grid(row=6, column=1, sticky='news')
                #self.txt.config(state=tk.DISABLED)
            elif idx_room>0:
                HOUSE = self.listbox_houses.get(first=idxs_house)
