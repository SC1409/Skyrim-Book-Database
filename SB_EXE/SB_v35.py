...
#OPEN LISTS OF HOUSES FOR MAIN WINDOW:
...
#OPEN LISTS OF HOUSES FOR ADD/MOVE BOOK WINDOWS:
...
#OPEN LISTS OF ROOMS FOR MAIN WINDOW:
allrooms_main = []
...
#OPEN LISTS OF ROOMS FOR ADD/MOVE BOOK WINDOW:
allrooms_add_move = []
...
#CREATE OPTIONS TO FLAG BOOK AS SKILL BOOK OR SPELL TOME:
...
class main_win:
    def __init__(self, master):
        ...
        #CONFIGURE ROWS AND COLUMNS OF MAIN FRAME TO ALLOW FOR EXPANSION:
        ...
        #CREATE FRAME FOR ADDBOOK WINDOW BUTTON:
        self.frame_addbook=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_addbook.grid(row=0, column=0, columnspan=2, sticky='news')
        #CONFIGURE ROWS AND COLUMNS OF ADDOOK BUTTON FRAME TO ALLOW FOR EXPANSION:
        self.frame_addbook.rowconfigure(0, weight=1)
        self.frame_addbook.columnconfigure(0, weight=1)
        #CREATE FRAME FOR MOVEBOOK WINDOW BUTTON:
        self.frame_movebook=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_movebook.grid(row=0, column=2, sticky='news')
        #CONFIGURE ROWS AND COLUMNS OF MOVEBOOK BUTTON FRAME TO ALLOW FOR EXPANSION:
        self.frame_movebook.rowconfigure(0, weight=1)
        self.frame_movebook.columnconfigure(2, weight=1)
        #CREATE FRAME FOR DELETEBOOK WINDOW BUTTON:
        self.frame_delbook=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_delbook.grid(row=0, column=3, sticky='news')
        #CONFIGURE ROWS AND COLUMNS OF DELETEBOOK BUTTON FRAME TO ALLOW FOR EXPANSION:
        self.frame_delbook.rowconfigure(0, weight=1)
        self.frame_delbook.columnconfigure(3, weight=1)
        #CREATE FRAME FOR SEARCH BUTTON:
        self.frame_search=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_search.grid(row=6, column=0, columnspan=2, sticky='news')
        #CONFIGURE ROWS AND COLUMNS OF SEARCH BUTTON FRAME TO ALLOW FOR EXPANSION:
        self.frame_search.rowconfigure(6, weight=1)
        self.frame_search.columnconfigure(0, weight=1)
        #CREATE FRAME FOR SKILL BOOK FILTER BUTTON:
        self.frame_skill=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_skill.grid(row=3, column=0, columnspan=2, sticky='news')
        #CREATE FRAME FOR SPELL TOME FILTER BUTTON:
        self.frame_spell=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_spell.grid(row=4, column=0, columnspan=2, sticky='news')
        #CREATE FRAME FOR REGULAR BOOK FILTER BUTTON:
        self.frame_reg=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_reg.grid(row=5, column=0, columnspan=2, sticky='news')
        #GET LIST OF HOUSES:
        ...
        self.listbox_rooms.bind('<ButtonRelease-1>', 'break')
        #CREATE TREEVIEW FOR TABLES:
        self.tree=ttk.Treeview(self.frame)
        self.tree.grid(row=1, column=2, rowspan=5, columnspan=2, sticky='news')
        self.tree.bind('<ButtonRelease-3>', self.tree_c2c)
        self.tree.bind('<Control-c>', self.tree_c2c)
        #STYLE OPTIONS FOR TREEVIEW
        ...
        self.btn_search=tk.Button(self.frame_search, text='Search for Title:', command=self.search, font=('Futura Condensed',15), bg='black', fg='white')
        self.btn_search.grid(row=6, column=0, columnspan=2, sticky='news')
        #CREATE BUTTON TO OPEN 'ADD BOOK' WINDOW:
        self.btn_open_addbook=tk.Button(self.frame_addbook, text='Add Book', command=self.open_addbook, font=('Futura Condensed',15), bg='black', fg='white')
        self.btn_open_addbook.grid(row=0, column=0, columnspan=2, sticky='news')
        #CREATE BUTTON TO OPEN 'MOVE BOOK' WINDOW:
        self.btn_open_movebook=tk.Button(self.frame_movebook, text='Move Book', command=self.open_movebook, font=('Futura Condensed',15), bg='black', fg='white')
        self.btn_open_movebook.grid(row=0, column=2, sticky='news')
        #CREATE BUTTON TO OPEN 'DELETE BOOK' WINDOW:
        self.btn_open_delbook=tk.Button(self.frame_delbook, text='Delete Book', command=self.open_delbook, font=('Futura Condensed',15), bg='black', fg='white')
    ...
    def forget_btns(self, *args):
        ...
        for item in lst:
            if isinstance(self.frame_skill, tk.Frame):
                self.frame_skill.grid_forget()
            if isinstance(self.frame_spell, tk.Frame):
                self.frame_spell.grid_forget()
            if isinstance(self.frame_reg, tk.Frame):
                self.frame_reg.grid_forget()
            else:
                pass

    #CREATE FUNCTION TO COPY TEXT FROM TEXTBOX TO CLIPBOARD:
    def tree_c2c(self, event):
        try:
            selected_txt = self.tree.item(self.tree.focus())
            column=self.tree.identify_column(event.x)
            if column=='#1':
                self.master.clipboard_clear()
                self.master.clipboard_append(selected_txt['values'][1])
            if column=='#2':
                self.master.clipboard_clear()
                self.master.clipboard_append(selected_txt['values'][2])
            if column=='#3':
                self.master.clipboard_clear()
                self.master.clipboard_append(selected_txt['values'][3])
        except tk.TclError:
            return
    ...
    #CREATE FUNCTION TO CUT TEXT FROM SEARCHBAR TO CLIPBOARD:
    def search_ct2c(self, *args):
        try:
            selected_txt = self.searchbar.selection_get()
            self.master.clipboard_clear()
            self.master.clipboard_append(selected_txt)
            self.master.update()
            self.searchbar.delete('sel.first', 'sel.last')
        except tk.TclError:
            return
    ...
    def search_pfc(self, *args):
        ...
        try:
            ...
            self.searchbar.insert('insert', content)
        except tk.TclError:
            return
    ...
    def all_skill(self, *args):
        ...
        self.tree.column('#3', stretch='YES', width=300, anchor=tk.CENTER)
    ...
    def addhouse_win(self, *args):
        ...
        #CREATE FRAME FOR ADDHOUSE BUTTON:
    ...
    def addhouse(self, *args):
        ...
        with open('C:/Users/steph/Documents/Python/SB_EXE/add_move_lists/lists_addmove_allrooms.txt', 'a+') as file_all_rooms_add_move:
            all_rooms_add_move=file_all_rooms_add_move.readlines()
    ...
    def fill_table_all_rooms(self, *args):         
        ...
        self.frame_reg=tk.Frame(self.frame, bg='black', highlightbackground = "white", highlightthickness=1)
        self.frame_reg.grid(row=5, column=0, columnspan=2, sticky='news')
        self.frame_skill.rowconfigure(3, weight=1)
        self.frame_skill.columnconfigure(0, weight=1)
        self.frame_spell.rowconfigure(4, weight=1)
        self.frame_spell.columnconfigure(0, weight=1)
        self.frame_reg.rowconfigure(5, weight=1)
        self.frame_reg.columnconfigure(0, weight=1)
        self.btn_search.grid(row=6, column=0, columnspan=2, sticky='news')
        self.btn_open_addbook.grid(row=0, column=0, columnspan=2, sticky='news')
        self.btn_open_movebook.grid(row=0, column=2, sticky='news')
        self.btn_open_delbook.grid(row=0, column=3, sticky='news')
        ...
        if len(idxs_rooms)==1:
            ...
            if idx_room==0:
                ...
                self.btn_open_movebook.grid(row=0, column=2, sticky='news')
                self.btn_open_delbook.grid(row=0, column=3, sticky='news')
                self.btn_all_skill=tk.Button(self.frame_skill, text='Skill Books', command=self.all_skill, font=('Futura Condensed',15), bg='black', fg='white')
                self.btn_all_skill.grid(row=3, column=0, columnspan=2, sticky='news')
                self.btn_all_spell=tk.Button(self.frame_spell, text='Spell Tomes', command=self.all_spell, font=('Futura Condensed',15), bg='black', fg='white')
                self.btn_all_spell.grid(row=4, column=0, columnspan=2, sticky='news')
                self.btn_all_reg=tk.Button(self.frame_reg, text='Spell Tomes', command=self.all_reg, font=('Futura Condensed',15), bg='black', fg='white')
    ...
    def fill_table(self, *args):
        ...
        if len(idxs_rooms)==1:
            ...
            if idx_room==0:
                ...
                for i in range(count):
                    self.tree.insert('', i, text=rowlabels[i], values=query.iloc[i,:].tolist())
                self.btn_all_room_skill=tk.Button(self.frame_skill, text='Skill Books', command=self.all_room_skill, font=('Futura Condensed',15), bg='black', fg='white')
                self.btn_all_room_skill.grid(row=3, column=0, columnspan=2, sticky='news')
                self.btn_all_room_spell=tk.Button(self.frame_spell, text='Spell Tomes', command=self.all_room_spell, font=('Futura Condensed',15), bg='black', fg='white')
                self.btn_all_room_spell.grid(row=4, column=0, columnspan=2, sticky='news')
                self.btn_all_room_reg=tk.Button(self.frame_reg, text='Regular Books', command=self.all_room_reg, font=('Futura Condensed',15), bg='black', fg='white')
                ...
                for i in range(count):
                    self.tree.insert('', i, text=rowlabels[i], values=query.iloc[i,:].tolist())
                self.btn_room_skill=tk.Button(self.frame_skill, text='Skill Books', command=self.room_skill, font=('Futura Condensed',15), bg='black', fg='white')
                self.btn_room_skill.grid(row=3, column=0, columnspan=2, sticky='news')
                self.btn_room_spell=tk.Button(self.frame_spell, text='Spell Tomes', command=self.room_spell, font=('Futura Condensed',15), bg='black', fg='white')
                self.btn_room_spell.grid(row=4, column=0, columnspan=2, sticky='news')
                self.btn_room_reg=tk.Button(self.frame_reg, text='Regular Books', command=self.room_reg, font=('Futura Condensed',15), bg='black', fg='white')
...
class addbook_win:
    def __init__(self, master):
        ...
        self.frame.rowconfigure(3, weight=1)
        self.frame.columnconfigure(0, weight=1)
        #CREATE FRAME FOR TITLE ENTRY:
        self.frame_title=tk.Frame(self.frame, highlightbackground = "white", highlightthickness=1)
        self.frame_title.grid(row=0, column=1, columnspan=3, sticky='news')
        #CONFIGURE ROWS AND COLUMNS OF TITLE ENTRY FRAME TO ALLOW FOR EXPANSION:
        self.frame_title.rowconfigure(0, weight=1)
        self.frame_title.columnconfigure(1, weight=1)
        #CREATE FRAME FOR ADDBOOK BUTTON:
        self.frame_btn=tk.Frame(self.frame, highlightbackground = "white", highlightthickness=1)
        self.frame_btn.grid(row=3, column=0, columnspan=4, sticky='ew')
        #CONFIGURE ROWS AND COLUMNS OF ADDBOOK BUTTON FRAME TO ALLOW FOR EXPANSION:
        ...
        self.add_title=tk.Entry(self.frame_title, textvariable=self.title, font=('Futura CondensedLight',15), bg='black', fg='white', insertbackground='white')
        ...
        self.btn_addbook=tk.Button(self.frame_btn, text='Add Book', command=lambda: [self.addbook(), self.master.destroy()], font=('Futura Condensed',15), bg='black', fg='white')
...
class movebook_win:
    def __init__(self, master):
        ...
        self.frame.rowconfigure(6, weight=1)
        self.frame.columnconfigure(0, weight=1)
        #CREATE FRAME FOR MOVEBOOK BUTTON:
        self.frame_btn=tk.Frame(self.frame, highlightbackground = "white", highlightthickness=1)
        self.frame_btn.grid(row=6, column=0, columnspan=4, sticky='ew')
        #CONFIGURE ROWS AND COLUMNS OF MOVEBOOK BUTTON FRAME TO ALLOW FOR EXPANSION:
        self.frame_btn.rowconfigure(6, weight=1)
        self.frame_btn.columnconfigure(0, weight=1)
        #CREATE LISTBOX FOR LIST OF TITLES:
        all_books=pd.read_sql_query('SELECT * FROM "My Books"', con)
        ...
        self.btn_movebook=tk.Button(self.frame_btn, text='Move Book', command=lambda: [self.movebook(), self.master.destroy()], font=('Futura Condensed',15), bg='black', fg='white')
    ...
    #CREATE FUNCTION TO MOVE BOOK IN DATABASE:
...
class delbook_win:
    def __init__(self, master):
        ...
        self.frame.rowconfigure(3, weight=1)
        self.frame.columnconfigure(0, weight=1)
        #CREATE FRAME FOR DELETEBOOK BUTTON:
        self.frame_btn=tk.Frame(self.frame, highlightbackground = "white", highlightthickness=1)
        self.frame_btn.grid(row=3, column=0, columnspan=4, sticky='ew')
        #CONFIGURE ROWS AND COLUMNS OF DELETEBOOK BUTTON FRAME TO ALLOW FOR EXPANSION:
        self.frame_btn.rowconfigure(3, weight=1)
        self.frame_btn.columnconfigure(0, weight=1)#
        #CREATE LISTBOX FOR LIST OF TITLES:
        all_books=pd.read_sql_query('SELECT * FROM "My Books"', con)
        self.lbl_select_title=tk.Label(self.frame, text='Select Book:', font=('Futura Condensed',15), bg='black', fg='white')
        ...
        self.btn_delbook=tk.Button(self.frame_btn, text='Delete Book', command=lambda: [self.delbook(), self.master.destroy()], font=('Futura Condensed',15), bg='black', fg='white')
