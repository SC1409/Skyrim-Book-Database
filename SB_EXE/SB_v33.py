...
font_btns_path='C:\\Users\\steph\\Documents\\Python\\SB_EXE\\13_SkyrimBooks_Gaelic.ttf'
...
class main_win:
    def __init__(self, master):
        ...
        self.lbl_houses=tk.Label(self.frame, text='Select House:', font='SkyrimBooks_Gaelic')
        self.lbl_houses.grid(row=1, column=0, sticky='ew')
        self.listbox_houses=tk.Listbox(self.frame, listvariable=Houses_main, bg='white', fg='black', exportselection=False, width=30, height=5)
        self.listbox_houses.config(font='SkyrimBooks_Handwritten_Bold')
        ...
        self.lbl_rooms=tk.Label(self.frame, text='Select Room:', font='SkyrimBooks_Gaelic')
        self.lbl_rooms.grid(row=1, column=1, sticky='ew')
        self.listbox_rooms=tk.Listbox(self.frame, bg='white', font='Ariel', fg='black', exportselection=False, width=30, height=5)
        self.listbox_rooms.config(font='SkyrimBooks_Handwritten_Bold')
        self.listbox_rooms.grid(row=2, column=1, sticky='ew')
        #CREATE TREEVIEW FOR TABLES:
        self.tree=ttk.Treeview(self.frame)
        self.style_tree=ttk.Style()
        self.style_tree.configure('Treeview', font='SkyrimBooks_Handwritten_Bold')
        #self.tree.config(font='SkyrimBooks_Handwritten_Bold')
        ...
        self.searchbar=tk.Entry(self.frame, textvariable=self.searchstring, font='SkyrimBooks_Handwritten_Bold')
        ...
        self.btn_search=tk.Button(self.frame, text='Search', command=self.search, font='SkyrimBooks_Gaelic')
        self.btn_search.grid(row=6, column=3, sticky='nes')
        #CREATE BUTTON TO OPEN 'ADD BOOK' WINDOW:
        self.btn_open_addbook=tk.Button(self.frame, text='Add Book', command=self.open_addbook, font='SkyrimBooks_Gaelic')
        self.btn_open_addbook.grid(row=0, column=0, columnspan=2, sticky='news')
        #CREATE BUTTON TO OPEN 'MOVE BOOK' WINDOW:
        self.btn_open_movebook=tk.Button(self.frame, text='Move Book', command=self.open_movebook, font='SkyrimBooks_Gaelic')
        self.btn_open_movebook.grid(row=0, column=2, sticky='news')
        #CREATE BUTTON TO OPEN 'DELETE BOOK' WINDOW:
        self.btn_open_delbook=tk.Button(self.frame, text='Delete Book', command=self.open_delbook, font='SkyrimBooks_Gaelic')
    ...
    def addhouse_win(self, *args):
        ...
        self.lbl_add_house=tk.Label(self.frame_addhouse, text='Add House:', font='SkyrimBooks_Gaelic')
        self.lbl_add_house.grid(row=0, column=0, columnspan=3, sticky='ew')
        self.addhouse_bar=tk.Entry(self.frame_addhouse, textvariable=self.new_house, font='SkyrimBooks_Handwritten_Bold')
        self.addhouse_bar.grid(row=1, column=0, columnspan=3, sticky='ew')
        #CREATE LABEL AND TEXTBOX FOR ROOM 1 NAME:
        self.lbl_add_room1=tk.Label(self.frame_addhouse, text='Add First Room:', font='SkyrimBooks_Gaelic')
        self.lbl_add_room1.grid(row=2, column=0, columnspan=3, sticky='ew')
        self.addroom1_bar=tk.Entry(self.frame_addhouse, textvariable=self.room_one, font='SkyrimBooks_Handwritten_Bold')
        self.addroom1_bar.grid(row=3, column=0, columnspan=3, sticky='ew')
        #CREATE LABEL AND TEXTBOX FOR ROOM 2 NAME:
        self.lbl_add_room2=tk.Label(self.frame_addhouse, text='Add Second Room:', font='SkyrimBooks_Gaelic')
        self.lbl_add_room2.grid(row=4, column=0, columnspan=3, sticky='ew')
        self.addroom2_bar=tk.Entry(self.frame_addhouse, textvariable=self.room_two, font='SkyrimBooks_Handwritten_Bold')
        self.addroom2_bar.grid(row=5, column=0, columnspan=3, sticky='ew')
        #CREATE LABEL AND TEXTBOX FOR ROOM 3 NAME:
        self.lbl_add_room3=tk.Label(self.frame_addhouse, text='Add Third Room:', font='SkyrimBooks_Gaelic')
        self.lbl_add_room3.grid(row=6, column=0, columnspan=3, sticky='ew')
        self.addroom3_bar=tk.Entry(self.frame_addhouse, textvariable=self.room_three, font='SkyrimBooks_Handwritten_Bold')
        self.addroom3_bar.grid(row=7, column=0, columnspan=3, sticky='ew')
        #CREATE LABEL AND TEXTBOX FOR ROOM 4 NAME:
        self.lbl_add_room4=tk.Label(self.frame_addhouse, text='Add Fourth Room:', font='SkyrimBooks_Gaelic')
        self.lbl_add_room4.grid(row=8, column=0, columnspan=3, sticky='ew')
        self.addroom4_bar=tk.Entry(self.frame_addhouse, textvariable=self.room_four, font='SkyrimBooks_Handwritten_Bold')
        self.addroom4_bar.grid(row=9, column=0, columnspan=3, sticky='ew')
        #CREATE BUTTON TO EXECUTE COMMAND:
        self.btn_addhouse=tk.Button(self.frame_addhouse, text='Add House', command=lambda: [self.addhouse(), self.newWindow.destroy()], font='SkyrimBooks_Gaelic')
    ...
    def fill_table_all_rooms(self, *args):         
        ...
        if len(idxs_rooms)==1:
            ...
            if idx_room==0:
                ...
                self.btn_open_movebook.grid(row=0, column=2, sticky='news')
                self.btn_open_delbook.grid(row=0, column=3, sticky='news')
                self.btn_all_skill=tk.Button(self.frame, text='Skill Books', command=self.all_skill, font='SkyrimBooks_Gaelic')
                self.btn_all_skill.grid(row=3, column=0, columnspan=2, sticky='news')
                self.btn_all_spell=tk.Button(self.frame, text='Spell Tomes', command=self.all_spell, font='SkyrimBooks_Gaelic')
                self.btn_all_spell.grid(row=4, column=0, columnspan=2, sticky='news')
                self.btn_all_reg=tk.Button(self.frame, text='Spell Tomes', command=self.all_reg, font='SkyrimBooks_Gaelic')
    ...
    def fill_table(self, *args):
        ...
        if len(idxs_rooms)==1:
            ...
            if idx_room==0:
                ...
                for i in range(count):
                    self.tree.insert('', i, text=rowlabels[i], values=query.iloc[i,:].tolist())
                self.btn_all_room_skill=tk.Button(self.frame, text='Skill Books', command=self.all_room_skill, font='SkyrimBooks_Gaelic')
                self.btn_all_room_skill.grid(row=3, column=0, columnspan=2, sticky='news')
                self.btn_all_room_spell=tk.Button(self.frame, text='Spell Tomes', command=self.all_room_spell, font='SkyrimBooks_Gaelic')
                self.btn_all_room_spell.grid(row=4, column=0, columnspan=2, sticky='news')
                self.btn_all_room_reg=tk.Button(self.frame, text='Regular Books', command=self.all_room_reg, font='SkyrimBooks_Gaelic')
                ...
                for i in range(count):
                    self.tree.insert('', i, text=rowlabels[i], values=query.iloc[i,:].tolist())
                self.btn_room_skill=tk.Button(self.frame, text='Skill Books', command=self.room_skill, font='SkyrimBooks_Gaelic')
                self.btn_room_skill.grid(row=3, column=0, columnspan=2, sticky='news')
                self.btn_room_spell=tk.Button(self.frame, text='Spell Tomes', command=self.room_spell, font='SkyrimBooks_Gaelic')
                self.btn_room_spell.grid(row=4, column=0, columnspan=2, sticky='news')
                self.btn_room_reg=tk.Button(self.frame, text='Regular Books', command=self.room_reg, font='SkyrimBooks_Gaelic')
...
class addbook_win:
    def __init__(self, master):
        ...
        self.lbl_addbook_title=tk.Label(self.frame, text='Enter Title:', font='SkyrimBooks_Gaelic')
        self.lbl_addbook_title.grid(row=0, column=0, sticky='ew')
        self.title=tk.StringVar()
        self.add_title=tk.Entry(self.frame, textvariable=self.title, font='SkyrimBooks_Handwritten_Bold')
        ...
        self.lbl_houses=tk.Label(self.frame, text='Select House:', font='SkyrimBooks_Gaelic')
        self.lbl_houses.grid(row=1, column=0, sticky='ew')
        self.listbox_houses=tk.Listbox(self.frame, listvariable=Houses_add_move, bg='white', font='Ariel', fg='black', width=30, height=4)
        self.listbox_houses.config(font='SkyrimBooks_Handwritten_Bold')
        ...
        self.listbox_rooms=tk.Listbox(self.frame, bg='white', font='Ariel', fg='black', exportselection=False, width=30, height=4)
        self.listbox_rooms.config(font='SkyrimBooks_Handwritten_Bold')
        self.listbox_rooms.grid(row=1, column=3, sticky='ew')
        #CREATE LISTBOX AND LABEL FOR YES/NO OPTION FOR SKILL BOOKS:
        self.lbl_skill=tk.Label(self.frame, text='Skill Book:', font='SkyrimBooks_Gaelic')
        self.lbl_skill.grid(row=2, column=0, sticky='ew')
        self.lbsk=tk.StringVar(self.frame, value=list_binary_skill)
        self.listbox_skill=tk.Listbox(self.frame, listvariable=self.lbsk, bg='white', font='Ariel', fg='black', exportselection=False, height=2)
        self.listbox_skill.config(font='SkyrimBooks_Handwritten_Bold')
        self.listbox_skill.grid(row=2, column=1, sticky='ew')
        #CREATE LISTBOX AND LABEL FOR YES/NO OPTION FOR SPELL TOMES:
        self.lbl_spell=tk.Label(self.frame, text='Spell Book:', font='SkyrimBooks_Gaelic')
        self.lbl_spell.grid(row=2, column=2, sticky='ew')
        self.lbsp=tk.StringVar(self.frame, value=list_binary_spell)
        self.listbox_spell=tk.Listbox(self.frame, listvariable=self.lbsp, bg='white', font='Ariel', fg='black', exportselection=False, height=2)
        self.listbox_spell.config(font='SkyrimBooks_Handwritten_Bold')
        self.listbox_spell.grid(row=2, column=3, sticky='ew')
        #CREATE BUTTON TO ADD BOOK:
        self.btn_addbook=tk.Button(self.frame, text='Add Book', command=lambda: [self.addbook(), self.master.destroy()], font='SkyrimBooks_Gaelic')
    ...
    def addbook(self, *args):
        ...
        if cur.fetchone():
            ...
            self.lbl_warn=tk.Label(self.warn_frame, text='A book with this title already exists', font='SkyrimBooks_Gaelic')
            self.lbl_warn.grid(row=0, column=0, sticky='news')
            self.btn_back=tk.Button(self.warn_frame, text='Return', command=self.warn_message.destroy, font='SkyrimBooks_Gaelic')
...
class movebook_win:
    def __init__(self, master):
        ...
        self.frame.grid(row=0, column=0, sticky='news')
        all_books=pd.read_sql_query('SELECT * FROM "My Books"', con)
        self.lbl_select_title=tk.Label(self.frame, text='Select Book:', font='SkyrimBooks_Gaelic')
        ...
        self.listbox_title.config(font='SkyrimBooks_Handwritten_Bold')
        ...
        self.btn_movebook=tk.Button(self.frame, text='Move Book', command=lambda: [self.movebook(), self.master.destroy()], font='SkyrimBooks_Gaelic')
...
class delbook_win:
    def __init__(self, master):
        ...
        self.lbl_select_title=tk.Label(self.frame, text='Select Book:', font='SkyrimBooks_Gaelic')
        self.lbl_select_title.grid(row=0, column=0, columnspan=2, sticky='ew')
        self.listbox_title=tk.Listbox(self.frame, bg='white', font='Ariel', fg='black', exportselection=False, width=42)
        self.listbox_title.config(font='SkyrimBooks_Handwritten_Bold')
        self.listbox_title.grid(row=1, column=0, sticky='news')
        for title in all_books["Title"]:
            self.listbox_title.insert(tk.END, title)
        #CREATE BUTTON TO DELETE BOOK:
        self.btn_delbook=tk.Button(self.frame, text='Delete Book', command=lambda: [self.delbook(), self.master.destroy()], font='SkyrimBooks_Gaelic')
