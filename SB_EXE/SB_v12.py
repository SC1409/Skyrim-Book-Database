...
def search(*event):
    ...
    txt.grid(row=0, column=2, rowspan=5, columnspan=2, sticky='nsew')
    btn_search.grid(row=5, column=3, sticky='news')
    all_books=pd.read_sql_query('SELECT * FROM "My Books"', con)
    result=all_books.loc[all_books['Title'].str.contains(searchstring.get(), na=False, case=False)]
...
#CREATE LABEL FOR SEARCH BAR WIDGET:
lbl_search=tk.Label(main_screen, text="Search for Title:")
lbl_search.grid(row=5, column=1, sticky='ew')
...
btn_search=tk.Button(main_screen, text='Search', command=search)
btn_search.grid(row=5, column=3, sticky='news')
#################################################CREATE WIDGETS#####################################################
##################################################RUN PROGRAM#######################################################
#LAUNCH DATABASE:
